...
def process_strip(strip, strip_n, strip_y, strip_h, strip_x, strip_w):
    ...
    strip_name = strip_n[:35, :300]
    strip_stats = strip_gray[10:, 390:]
    strip_score = open_strip(strip_stats[5:, :100])
    cv.imshow("SCORE", strip_score)
    cv.waitKey(0)
    strip_goals = open_strip(strip_stats[5:, 115:175])
    strip_assists = open_strip(strip_stats[5:, 200:275])
    strip_saves = open_strip(strip_stats[5:, 320:380])
    strip_shots = open_strip(strip_stats[5:, 410:])
...
def main():
    ...
    for i in range(6):
        strip_x = 67
        strip_y = [5, 60, 120, 260, 320, 375][i]
        strip_w = 843
        strip_h = 40
